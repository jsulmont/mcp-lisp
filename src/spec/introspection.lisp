...
Covers all registered DSL forms with synopsis, example, and options."
  (with-output-to-string (out)
    ...
    (format out "- `(list-mixins)`, `(list-compounds)` — registered mixins and compound types~%")
    (format out "- `(diff-specs '(\"entity1\" \"entity2\" ...))` — compare entities against expected list~%")
    ...
    (format out "- `(random-walk-scenario \"scenario\" :steps 20 :trials 50)` — multi-entity scenario walk~%")
    ...
    (format out "~%### Built-in helpers~%~%")
    (format out "Available in invariant `:check` forms and `defhelper`/`defgenerator` bodies with no imports.~%~%")
    (format out "#### Collection predicates~%~%")
    (format out "- `(all-pairs-check lst pred)` — T if PRED holds for every unordered pair in LST~%")
    (format out "- `(consecutive-pairs-check lst pred)` — T if PRED holds for every consecutive pair in LST~%")
    (format out "~%#### Geospatial~%~%")
    (format out "- `(haversine-distance-nm lat1 lon1 lat2 lon2)` — great-circle distance in nautical miles~%")
    (format out "- `(initial-bearing-deg lat1 lon1 lat2 lon2)` — forward azimuth from point 1 to point 2 in degrees [0,360)~%")
    (format out "- `(heading-difference-deg h1 h2)` — smallest angular difference between two headings in degrees [0,180]~%")
    (format out "- `(point-in-polygon-p lat lon polygon)` — T if point is inside polygon; polygon is list of (lat lon) pairs~%")
    (format out "~%#### Temporal intervals~%~%")
    (format out "- `(intervals-overlap-p start1 dur1 start2 dur2)` — T if two intervals overlap~%")
    (format out "- `(interval-contains-p outer-start outer-dur inner-start inner-dur)` — T if inner interval is within outer~%")
    (format out "- `(interval-before-p start1 dur1 start2)` — T if interval ends at or before start2~%")
    (format out "~%#### Temporal durations~%~%")
    (format out "- `(elapsed-since timestamp now)` — elapsed time between two timestamps~%")
    (format out "- `(duration-at-least-p timestamp now min-duration)` — T if at least min-duration has elapsed~%")
    (format out "- `(within-retention-period-p event-time retention-duration now)` — T if now is within retention window~%")
    (format out "~%#### Generation strategies~%~%")
    (format out "- `(distribute-values n total :min 0)` — N non-negative numbers summing to at most TOTAL~%")
    (format out "- `(partition-into items n)` — randomly partition ITEMS into N groups~%")
    (format out "~%### Rule templates~%~%")
    (format out "Use CL `defmacro` to avoid repetitive `defrule` definitions.~%")
    (format out "Macros expand at eval time; expanded rules persist through serialization.~%")
    (format out "Macro definitions themselves do not survive JSON round-trips.~%")
    ...
    (format out "- `(load-spec-file #p\"/path/to/file.lisp\")` — load and compose specs from file~%")))
